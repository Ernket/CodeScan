...
- 稳定提交工具：新增 `submit_routes`、`submit_findings`、`submit_reviews`，让模型分批提交路由、漏洞和复核结论，避免一次性输出超长 JSON 被截断。
- 结构化查询工具：新增/强化 `query_routes`、`query_stage_output`、`query_manifest`，支持按页查询路由、阶段发现和项目线索，不需要把完整大结果反复塞回模型上下文。
...
- 多用户登录：支持账号密码登录、token 鉴权、账号启停、密码重置和组织授权控制。
...
本次更新把模型的“最终一次性返回大 JSON”改成了更稳定的分批提交流程，核心目标是尽量避免路由清单、漏洞列表或复核结论过长时被模型输出上限截断。

- `submit_routes`：仅在 `init` 路由发现阶段使用。模型每确认一批路由就提交 1-20 条，后端持续落盘、去重并在阶段结束时组装最终路由结果。
- `submit_findings`：在非 `init` 漏洞审计阶段使用。模型每确认一批漏洞就提交 1-5 条完整漏洞对象，后端保存中间结果，阶段结束后统一生成 `OutputJSON`。
- `submit_reviews`：在复核阶段使用。模型按 `finding_index` 分批提交 1-20 条复核结论，只覆盖 `verification_status`、`reviewed_severity` 和 `verification_reason`，避免复核时重写整条漏洞详情。
- `query_routes`：按 `method`、`path`、`source`、分页参数查询当前结构化路由清单，适合 gap-check 或后续阶段按需读取路由子集。
- `query_stage_output`：按阶段分页查询已有漏洞输出，复核时会返回稳定的 `finding_index`，用于精确提交对应漏洞的复核结论。
- `query_manifest`：查询轻量项目清单、模块根、候选路由文件和阶段热点，帮助模型优先定位高价值文件，而不是盲目展开整个项目。

这些工具会把已提交内容保存在任务运行时状态中；即使模型后续输出异常或被截断，已经成功提交的路由、漏洞或复核结论也会被保留，便于重试和修复输出。
...
- 普通用户没有系统角色，项目权限严格来自组织授权。
- 组织 `member` 授权：可以读取已授权组织及其子组织的项目。
- 组织 `admin` 授权：可以在已授权组织及其子组织范围内上传项目、启动扫描、阶段重跑、复核和修复输出；不包含用户管理、组织管理或项目删除权限。
- 组织授权会沿组织树向子组织生效，方便按团队、部门或客户空间划分审计项目。
...
发布时先构建前端，再使用 `embedded_frontend` 标签编译后端，可得到一个同时服务 Vue 页面和 `/api` 接口的可执行文件。Windows 下可以直接执行仓库根目录的打包脚本：

```bat
build-release.bat
```

脚本会自动构建前端，并输出 Windows 与 Linux 两套包到 `releases/`：

- `CodeScan-windows-amd64.zip`
- `CodeScan-linux-amd64.tar.gz`
